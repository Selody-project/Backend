    ...
    try {
        ...
        if (req?.files?.image) {
            ...
            const fileName = transliterate(
                sanitizeFileName(req.files.image.name)
            );
...
async function uploadPostMiddleware(req, res, next) {
    try {
        ...
        for (const fileKey of fileKeys) {
            ...
            await s3Client.send(command);
            fileUrl.push(
                `https://${bucketName}.s3.amazonaws.com/${bucketParams.Key}`
            );
        }
        req.fileUrl = fileUrl;
        return next();
    ...
    try {
        if (
            ...
            for (const fileUrl of fileUrls) {
                if (
                    fileUrl === process.env.DEFAULT_USER_IMAGE ||
                    fileUrl === process.env.DEFAULT_GROUP_IMAGE ||
                    fileUrl === ""
                ) {
                    /* eslint-disable-next-line */
                    continue;
